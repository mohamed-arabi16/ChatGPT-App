...
import { buildEligibilitySnapshot, BuildEligibilitySnapshotInput } from '../../src/tools/build-eligibility-snapshot';
import { buildDocumentChecklist, BuildDocumentChecklistInput } from '../../src/tools/build-document-checklist';
import { buildTimeline, BuildTimelineInput } from '../../src/tools/build-timeline';
...
describe('Golden Test Scenarios', () => {
  ...
  describe('Degree Level Scenarios', () => {
    it('Scenario: High school graduate seeking bachelor in Computer Science', () => {
      const searchInput: SearchProgramsInput = {
        profile: {
      ...
      const searchResult = searchPrograms(searchInput, repository);
  ...
  describe('Language Preference Scenarios', () => {
    it('Scenario: English-only programs', () => {
      ...
      expect(result.data?.programs.every(p => 
        p.language === 'en' || p.language === 'mixed'
      )).toBe(true);
    });
    ...
    it('Scenario: Turkish-only programs', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.programs.every(p => 
        p.language === 'tr' || p.language === 'mixed'
      )).toBe(true);
    });
  });
  ...
  describe('Eligibility Scenarios', () => {
    ...
    it('Scenario: Unqualified student (low GPA)', () => {
      ...
      expect(result.data?.reasons.some(r => 
        r.rule_type === 'gpa_minimum' && !r.passed
      )).toBe(true);
    });
  ...
  describe('Document Checklist Scenarios', () => {
    it('Scenario: Bachelor program documents', () => {
      ...
      expect(result.data?.items.some(i => 
        i.doc_key.includes('high_school')
      )).toBe(true);
    });
    ...
    it('Scenario: Documents have required priority markers', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.items.every(i => 
        i.priority === 'required' || i.priority === 'recommended'
      )).toBe(true);
    });
  });
  ...
  describe('End-to-End Journey Scenarios', () => {
    it('Complete journey: Egyptian student seeking CS bachelor', async () => {
      const profile = {
        nationality: 'Egyptian',
        current_education_level: 'high_school' as const,
        desired_degree_level: 'bachelor' as const,
        major_keywords: ['computer'],
      ...
      const searchResult = searchPrograms({ profile }, repository);
      expect(searchResult.success).toBe(true);
      ...
      const eligibilityResult = buildEligibilitySnapshot({
      ...
      const checklistResult = buildDocumentChecklist({
        profile,
        program_id: programId,
      }, repository);
    ...
    it('Complete journey: Saudi student seeking MBA', async () => {
      const profile = {
        nationality: 'Saudi',
        current_education_level: 'bachelor' as const,
        desired_degree_level: 'master' as const,
        major_keywords: ['business', 'mba'],
        preferred_language: 'en' as const,
      };
  ...
  describe('Data Integrity Scenarios', () => {
    it('All programs should have verification status', () => {
      ...
      expect(result.data?.programs.every(p => 
        ...
        p.verification_status === 'outdated'
      )).toBe(true);
    });
...
function seedComprehensiveTestData(db: Database.Database) {
  // Insert universities
  db.prepare(`
